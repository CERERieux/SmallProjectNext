...
export default function UncontrolledInput({
  ...
  let localInput: HTMLElement | null = null;
  if (typeof document !== "undefined")
    localInput = document.getElementsByName(name)[0]; // Auxiliar to get the input by name
  const invalidStyles =
  ...
  const longText =
    canBeTooLong && size != null && currentValue.length > size - 3
  ...
  useEffect(() => {
    if (localInput !== undefined) {
      ...
      if (defaultValue !== "" && !modified.current) {
